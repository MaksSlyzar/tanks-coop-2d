...
import leaderIconSvg from "./../assets/leader-icon.svg";
import readyIconSvg from "./../assets/ready-icon.svg";
...
class RoomView extends View {
  ...
  constructor() {
    ...
    SIOManager.socket.on("updateRoomViewData", (roomData) =>
      this.updateRoomViewData(roomData)
    );
  ...
  updateRoomViewData = (roomData: any) => {
    const gameNameDiv = getElementByClass<HTMLDivElement>(
      "RoomView__game-name",
      this.mainDiv
    );
    const rolesDiv = getElementByClass<HTMLDivElement>(
      "RoomView__SelectRole-list",
      this.mainDiv
    );
    ...
    roles.map((role) => {
      ...
      if (this.selectedRole == role._type) roleDiv.classList.add("activated");
  ...
  startGame = () => {
    if (this.canStartGame == true) SIOManager.socket.emit("startRoomGame", {});
  };
  ...
  ready = (_ready: boolean) => {
    ...
    this.roomReadyButton.innerText = this.clientReady
      ? "Готовий"
      : "Не готовий";

    SIOManager.socket.emit("switchRoomReady", {
  ...
  updatePlayerList(players: any) {
    const playersDiv = getElementByClass<HTMLDivElement>(
      "RoomView__Players-list",
      this.mainDiv
    );
    const isRoomLeader = players.find(
      (pl: any) => pl.id == SIOManager.playerId
    ).isRoomLeader;

    playersDiv.innerHTML = "";

    let playersReady = true;
    const leaderIconImage = getImageSvg(leaderIconSvg, 20, 20, "leader-icon");
    const readyIconImage = getImageSvg(readyIconSvg, 20, 20, "ready-icon");
    ...
    players.map(
      (pl: {
        ...
        if (pl.roomReady == false) playersReady = false;
        const elem = document.createElement("div");

        elem.innerHTML = `${pl.username} - ${
          pl.isOnline ? "Онлайн" : "Офлайн"
        } `;

        if (pl.roomReady) elem.appendChild(readyIconImage);
        if (pl.isRoomLeader) elem.appendChild(leaderIconImage);
